REM $Id: lopu.8i.pkb,v 1.1 2001/11/30 23:24:55 bill Exp $
REM From "Learning Oracle PL/SQL" page 333

REM Body of lopu package that adds random value generator (for use with
REM Oracle8i; see also the file lopu.pkb) and password support
...
AS
   ...
   BEGIN
      ...
      THEN
         seedval := TO_CHAR(SYSDATE, 'SS');

         OPEN stcur;
         FETCH stcur INTO l_statsum;
         CLOSE stcur;

         seedval := seedval || TO_CHAR(l_statsum);
         DBMS_RANDOM.SEED(seedval);
         random_seeded := TRUE;

      END IF;
      RETURN DBMS_RANDOM.STRING(OPT => 'l' /* lower case */, LEN => 45);
   END;

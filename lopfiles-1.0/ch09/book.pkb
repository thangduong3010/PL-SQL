REM $Id: book.pkb,v 1.1 2001/11/30 23:24:55 bill Exp $
REM From "Learning Oracle PL/SQL" page 350

REM Body of ch09's version of book package that adds security features
...
AS
   PROCEDURE add(isbn_in IN VARCHAR2, title_in IN VARCHAR2,
      ...
      barcode_id_in IN VARCHAR2, requestor_id IN lib_users.id%TYPE)
   IS
   BEGIN
      priv.assert_allowed(requestor_id, priv.edit_book_c);
      ...
      THEN
         add_copy(isbn_in, barcode_id_in, requestor_id);
      END IF;

   END add;

   PROCEDURE add_copy(isbn_in IN VARCHAR2, barcode_id_in IN VARCHAR2,
      requestor_id IN lib_users.id%TYPE) IS
   BEGIN
      priv.assert_allowed(requestor_id, priv.edit_book_copy_c);
      lopu.assert_notnull(isbn_in);
   ...
   PROCEDURE change(isbn_in IN VARCHAR2, new_title IN VARCHAR2,
      ...
      requestor_id IN lib_users.id%TYPE)
   IS
   BEGIN
      priv.assert_allowed(requestor_id, priv.edit_book_c);
      lopu.assert_notnull(isbn_in);
   ...
   IS
   BEGIN
      priv.assert_allowed(requestor_id, priv.delete_book_copy_c);
      lopu.assert_notnull(barcode_id_in);
      lopu.assert_notnull(isbn_in);
      DELETE book_copies
   ...
   IS
   BEGIN
      priv.assert_allowed(requestor_id, priv.weed_book_c);
      lopu.assert_notnull(isbn_in);

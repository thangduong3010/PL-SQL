...
Rem
Rem    NAME
Rem      utlnchar.sql - The migration script for NCHAR data from 8i to 9i
Rem
Rem    DESCRIPTION
Rem      The migration script for NCHAR data from 8i to 9i 
Rem
Rem    NOTES
Rem      1) use SQLPLUS 
...
Rem      nchar, nvarchar2 and nclob, from 8i and 9i to make NCHAR
Rem      data accessible in 9i.  It should run after database is 
Rem      upgraded from 8i to 9i.
Rem      Once the script runs, what has been done can not be undone.
Rem
...
Rem    yzhu        04/13/01 - Add one more check for old NCHAR data.
Rem    yzhu        04/06/01 - Created
Rem

prompt
prompt Warning:
prompt The script is to migrate NCHAR data, including nchar
prompt nvarcha2 and nclob, from 8i to 9i.
prompt Once it runs, it can not undo what have been done.
prompt
prompt To run it, 1) use SQLPLUS  and 2) connect AS SYSDBA
prompt
...
create or replace procedure new_nchar (
  ...
  cursor c is
    select name, type#, length from col$
        where obj# = table_obj# and charsetform=2 and type# in (1, 96, 112)
                     and bitand(property, 8388608) = 0;
...
rem--------------------------------------------------------
rem Migrate old NCHAR data to new NCHAR data
rem--------------------------------------------------------
...
declare
  ...
  cursor c is
    select obj# from obj$
        where type# = 2 and owner# not in ( select user_id from all_users
                    where username='SYS' or username='SYSTEM');

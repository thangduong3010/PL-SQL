...
Rem     mmuppago   10/03/05  - Bump up the versionfor bug fix 4608020: 
...
Rem     pkapasi    05/29/03 -  Fix bug#2980038, bug#2807392, bump version
...
Rem     skwong     08/24/01 -  Add calls to setHTTPcharset and fix NL_CHAR
...
create or replace package body OWA_UTIL is
   ...
   owa_version CONSTANT varchar2(64) := '10.1.2.0.6';
  ...
  /* Constant is set here instead of owachars to avoid invalid objects */
   ...
   procedure signature(cname in varchar2) is
   begin
      signature;
   ...
   function is_synonym_checked(str in varchar2)
   return boolean
   is
   begin
       if checked_synonyms is not null
          and checked_synonyms.count > 1
       then 
           for i in checked_synonyms.first..checked_synonyms.last
           loop
               if (checked_synonyms.exists(i) and checked_synonyms(i) = str)
	       then
                   return true;
               end if; 
           end loop;
   ...
   begin
      if ( (get_synonym_defn(csynonym, cschema, name, owner, db_link)
             ...
             and  not is_synonym_checked(name||'.'||owner))
      then
         if checked_synonyms is null 
         then 
            checked_synonyms(1):= name||'.'||owner;
         else
            checked_synonyms(checked_synonyms.count + 1):= name||'.'||owner;
         end if; 
         if (db_link is null)
         then
   ...
   begin
      ...
      then
	    ...
	    -- remove trailing ',' from col_in_clause
            ...
            dbms_sql.define_column(col_cursor, 1, col_name, 255);
            dbms_sql.define_column(col_cursor, 2, col_dtype, 9);
            dbms_sql.define_column(col_cursor, 3, col_num, 9);
   ...
   begin
      ...
      then
         ...
         loop
            ...
            then
               col_sizes(col_counter):= nvl(lengthb(col_aliases(col_counter)),0);
            else
         ...
         then
            ...
            loop
               ...
               then
                  ...
                  tableData(date_var, col_sizes(col_counter), 'LEFT', row_string, ntable_type);
               elsif (col_dtype = 'LONG') then
   ...
   begin
      ...
      if (ccol_aliases is not null)
      then
         comma_to_ident_arr(ccol_aliases, col_aliases, num_aliases);
      else
         num_aliases := 0;
      end if;

      calc_col_sizes(ctable, ntable_type, ccolumns, col_names, col_dtypes,
                     nnum_cols, col_aliases, num_aliases, cclauses,
   ...
   /******************************************************************/
	...
	procedure format_cell(
           columnValue in varchar2, format_numbers in varchar2
        ) is
	begin
	...
	--
	    ...
	    loop
	        ...
	        htp.formSelectOption( cvalue => visible, 
	               cselected => ite( selected IS NULL, NULL, 'SELECTED' ),
   ...
   /******************************************************************/
	...
	begin
		...
		htp.tableCaption( to_char( l_start, 'Month YYYY' ) );
	
		for i in l_loop_start .. l_loop_stop loop
			htp.tableHeader( cvalue => to_char( l_magic_date+i, 'Day' ),
							 cattributes => l_width );
		end loop;
		htp.tableRowOpen;
		loop
			exit when to_char( l_magic_date, 'DY' ) = to_char(l_start,'DY');
		...
		loop
			exit when ( to_char( p_start,'MON') <> to_char( l_start,'MON') );
	
			if ( not l_mf_only or not is_weekend(l_start) ) then
				htp.p( '<td valign="TOP" '  || l_width || '>' );
				htp.p( htf.italic(htf.bold(to_char(l_start,'DD'))) || htf.br );
			end if;
		
			while(l_cnt < p_cnt AND to_char(l_start) = to_char(p_dates(l_cnt)) )
			loop
				if ( not l_mf_only or not is_weekend(l_start) ) then
					htp.p( '&#187;' );
	...
	begin
		...
		loop
			...
			if (l_yyyymon is null)
                        then
                           l_yyyymon := to_char(l_curr_date, 'YYYYMON');
                        end if;
	
			if (to_char(l_curr_date, 'YYYYMON') <> l_yyyymon)
                        then
			   show_internal( p_mf_only, l_dates(0), 
			      l_dates, l_text, l_link, l_cnt );
			   l_cnt := 0;
			   l_yyyymon := to_char(l_curr_date, 'YYYYMON');
			end if;
		...
		if (l_cnt > 0)
                then
                   show_internal(p_mf_only,
                      l_dates(0), l_dates, l_text, l_link, l_cnt);
		end if;
	end calendarprint;
